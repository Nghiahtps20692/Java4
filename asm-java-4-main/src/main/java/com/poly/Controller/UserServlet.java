...
@WebServlet(urlPatterns = { "/login", "/logout", "/register", "/forgotPass","/changePass","/editprofile" })
...
//		String id   = userService.findByUsername(user.getUsername());
//				//historyService.findByUserAndIsLiked(user.getUsername());
//		User item = userService.findByID(null);
//		 req.setAttribute("user", item);
//		RequestDispatcher requestDispatcher = req.getRequestDispatcher("/views/user/Editprofiler.jsp");
//		requestDispatcher.forward(req, res);
//	}
	
	private void ddoGetedit(HttpSession session, HttpServletRequest req, HttpServletResponse res)
			throws ServletException, IOException {
		User user = (User) session.getAttribute(SessionAtt.CURRENT_USER);
		User id   = userService.findByUsername(user.getUsername());
				//historyService.findByUserAndIsLiked(user.getUsername());
		 req.setAttribute("user", id);
//		RequestDispatcher requestDispatcher = req.getRequestDispatcher("/views/user/Editprofiler.jsp");
//		requestDispatcher.forward(req, res);
		req.getRequestDispatcher("/views/user/Editprofile.jsp").forward(req, res);
	}
	...
	private void doPostedit(HttpSession session, HttpServletRequest req, HttpServletResponse res)
			throws ServletException, IOException {
